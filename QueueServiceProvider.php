...
use Fabricate\Queue\Connectors\BeanstalkdConnector;
...
use Fabricate\Queue\Connectors\SqsConnector;
use Fabricate\Queue\Connectors\SyncConnector;
use Fabricate\Queue\Failed\DatabaseFailedJobProvider;
use Fabricate\Queue\Failed\DatabaseUuidFailedJobProvider;
use Fabricate\Queue\Failed\DynamoDbFailedJobProvider;
...
class QueueServiceProvider extends ServiceProvider implements DeferrableProvider
{
    ...
    {
        $this->program->singleton('queue', function ($app) {
    ...
    {
        $this->program->singleton('queue.connection', function ($app) {
    ...
    protected function registerFailoverConnector($manager)
    {
        $manager->addConnector('failover', function () use ($manager) {
            return new FailoverConnector(
                $manager,
                $this->program->make(EventDispatcher::class)
            );
        });
    }
    ...
    protected function registerDatabaseConnector($manager)
    {
        $manager->addConnector('database', function () {
            return new DatabaseConnector($this->program['db']);
        });
    }
    ...
    protected function registerRedisConnector($manager)
    {
        $manager->addConnector('redis', function () {
            return new RedisConnector($this->program['redis']);
        });
    }
    ...
    {
        $this->program->singleton('queue.worker', function ($app) {
            $isDownForMaintenance = function () {
                return $this->program->isDownForMaintenance();
            };
    ...
    {
        $this->program->singleton('queue.listener', function ($app) {
    ...
    {
        $this->program->singleton('queue.routes', function () {
    ...
    {
        $this->program->singleton('queue.failer', function ($app) {
            ...
            if (isset($config['driver']) && $config['driver'] === 'file') {
                return new FileFailedJobProvider(
                    $config['path'] ?? $this->program->storagePath('framework/cache/failed-jobs.json'),
                    $config['limit'] ?? 100,
     ...
     *
     * @param  array  $config
     * @return \Fabricate\Queue\Failed\DatabaseFailedJobProvider
     */
    protected function databaseFailedJobProvider($config)
    {
        return new DatabaseFailedJobProvider(
            $this->program['db'], $config['database'], $config['table']
        );
    }
     ...
     *
     * @param  array  $config
     * @return \Fabricate\Queue\Failed\DatabaseUuidFailedJobProvider
     */
     ...
     *
     * @param  array  $config
     * @return \Fabricate\Queue\Failed\DynamoDbFailedJobProvider
     */
